...
class DQN:
    ...
    def get_second(self):
        ...
        move_values = [[move, value] for move, value in zip(values, moves)]
    ...
    def train(self, replay_0, replay_1, games_at_once = 4, epsilon = 0.5, train_interval = 4, train_batch = 32, transfer_interval = 8192, episodes = 400000):
        ...
        while term_count < episodes:
            ...
            for state in current_states:
                move_value = []
                for move in moves:
                    move_value.append([move, values[27 * move[0] + 9 * move[1] + 3 * move[2] + move[3]]])
            ...
            current_rewards = [(s.payoff() if t else 0) for s, t in zip(new_states, terminal_0)] # 0
            for s, a, s2, r, t in zip(current_states, current_actions, new_states, current_rewards, terminal_0):
                if t:
                    replay_0.add(s, a, s2, r)
            # person 1 updates 
            if previous_states != []:
                for s, a, s1, s2, r in zip(previous_states, previous_actions, current_states, new_states, previous_rewards):
                    if not is_terminal(*s):
                        if is_terminal(*s1):
                            replay_1.add(s, a, s2, -r)
                        else:
                            replay_1.add(s, a, s2, -r)
            ...
            move_values = []
            for state in new_states_2:
                move_value = []
                for move in moves:
                    move_value.append([move, values[27 * move[0] + 9 * move[1] + 3 * move[2] + move[3]]])
            update_actions = [select(move_value) for move_value in move_values]
            update_states = [successor(action, *state) for action, state in zip(update_actions, update_states)]
            terminal_1 = [is_terminal(*state) for state in update_states]
            update_rewards = [(s.payoff() if t else 0) for s, t in zip(update_states, terminal_1)] # 1
