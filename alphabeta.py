...
def minimax_w_alpha_beta(state, depth, alpha, beta, maximizing_player):
    if depth == 0 or state.is_terminal():
        ...
        return evaluate_state(state), move
...
def evaluate_state(state):
    ...
    for i in range(3):
        for j in range(3):
            ...
            for k in range(3):
                # Rows
                if board[k][0] == board[k][1] == state.current_player and board[k][2] == -1:
                    score += 10
                if board[k][1] == board[k][2] == state.current_player and board[k][0] == -1:
                    score += 10
                if board[k][0] == board[k][2] == state.current_player and board[k][1] == -1:
                    score += 10
                # Columns
                if board[0][k] == board[1][k] == state.current_player and board[2][k] == -1:
                    score += 10
                if board[1][k] == board[2][k] == state.current_player and board[0][k] == -1:
                    score += 10
                if board[0][k] == board[2][k] == state.current_player and board[1][k] == -1:
                    score += 10
            # Diagonals
            if board[0][0] == board[1][1] == state.current_player and board[2][2] == -1:
                score += 15
            if board[1][1] == board[2][2] == state.current_player and board[0][0] == -1:
                score += 15
            if board[0][2] == board[1][1] == state.current_player and board[2][0] == -1:
                score += 15
            if board[2][0] == board[1][1] == state.current_player and board[0][2] == -1:
                score += 15
